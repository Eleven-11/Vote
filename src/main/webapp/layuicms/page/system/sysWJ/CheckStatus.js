layui.config({
    ...
    form.on("submit(tijiao)", function (data) {
        ...
        // alert(content);
        var optionIds = [];
        var object1 = document.getElementsByClassName("radio_");
        for (var p in object1){
            if (object1[p].checked){
                optionIds.push(object1[p].value);
            }
        }
        var object = document.getElementsByClassName("checkbox_");
        for (var p in object){
            if (object[p].checked){
                optionIds.push(object[p].value);
            }
        }
