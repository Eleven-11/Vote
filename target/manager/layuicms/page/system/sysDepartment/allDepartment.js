...
layui.config({
     ...
     * 填充组织机构信息
     * */
    function fillOrgInfo(departmentId) {
        var url = $tool.getContext() + 'depart/get.do';
        var req = {
        ...
        $api.Getorg(req,function (res) {
            var info = res.data;
            $('[name="departmentId"]').val(info.departmentId);
            $('[name="departmentName"]').val(info.departmentName);
    ...
    form.on("submit(editOrg)", function (data) {
        ...
        var url = $tool.getContext() + 'depart/update.do';
        var req = {
            departmentId: departmentId,
    ...
    form.on("submit(delOrg)", function (data) {
        ...
        var url = $tool.getContext() + 'depart/delete.do';
        var req = {
            departmentId: departmentId
        };
    ...
    form.on("submit(queryOrg)", function (data) {
        ...
        var find = findCursively(departmentName, orgNodes);
        if (find) {
            // 设置展开
     ...
     * 设置展开
     * */
    function setExpand(superiorDepartment,nodes) {
        var resNodes = $tool.jsonClone(nodes); // 拷贝一份，避免原数组被修改
        var paths = superiorDepartment.split('#');
